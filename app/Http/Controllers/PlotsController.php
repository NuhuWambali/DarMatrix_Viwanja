...
class PlotsController extends Controller
{
    ...
    public function addPlot(Request $request){
        $validatedPlotData=$request->validate([
            ...
            'installment_price_per_sqm'=>'required|numeric|min:0',
            'status'=>'required',
        ]);
            ...
            $addPlot->installment_price_per_sqm =$validatedPlotData['installment_price_per_sqm'];
            ...
            $installment_total=intval($validatedPlotData['plot_size']) *  intval($validatedPlotData['installment_price_per_sqm']);
            $addPlot->installment_total_value=$installment_total;
            $addPlot->monthly_installment_price =intval($installment_total/$project->installment_period);
    ...
    public function createPlotEdit(Request $request,$id){
        $validatedPlotData=$request->validate([
            'plot_number'=>'required|string|min:0',
            'plot_size'=>'required|numeric|min:0',
            'land_use'=>'required',
        ...
        $editPlot->installment_price_per_sqm =$project->price_per_sqm;
        ...
        $editPlot->installment_total_value = $validatedPlotData['monthly_installment_price'] * $project->installment_period;
    ...
    public function getPlots($projectId)
    {
        ...
        $plots = $project->plots->where('status',1);
